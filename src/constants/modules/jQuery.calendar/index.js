...
(function($) {
    var defaults = {
        ...
        DAY_ITEM_TPL =
            "<li " + ITEM_STYLE + ' class="{class}" {action}><div class="cal-item">{value}</div></li>',
        MONTH_ITEM_TPL = "<li " + ITEM_STYLE + " " + ITEM_MONTH + "><div class='cal-item'>{m}月</div></li>",
        TEMPLATE = [
            ...
            '<span class="calendar-prev" title="上一月" data-calendar-arrow-date>{prev}</span>',
            '<span class="calendar-next" title="下一月" data-calendar-arrow-date>{next}</span>',
            "</div>",
            "</div>",
            '<div class="calendar-ct">',
            '<ol class="week">{week}<div class="clearfix"></div></ol>',
            ...
            '<div class="calendar-arrow">',
            '<span class="calendar-prev" title="上一年" data-calendar-arrow-month>{prev}</span>',
            '<span class="calendar-next" title="下一年" data-calendar-arrow-month>{next}</span>',
            "</div>",
            "</div>",
    ...
    Calendar.prototype = {
        ...
        getDaysHtml: function(y, m) {
            ...
            var prevDaysNum = Date.getDaysNum(year, prevM.m);
        ...
        updateDisDate: function(y, m) {
            this.$disDate.html(DATE_DIS_TPL.repeat({ year: String(y).replace("年", ""), month: m }));
        },
        updateDisMonth: function(y) {
            this.$disMonth.text(y + "年");
        },
        ...
        trigger: function() {
            ...
            var _this = this,
                $this = _this.$element,
                post = ["relative", "absolute", "fixed"].indexOf(_this.$trigger.css("position")) > -1 ? _this.$trigger.position() : _this.$trigger.offset(),
        ...
        getDisDateValue: function() {
            var arr = this.$disDate.text().split("年"),
                y = Number(arr[0]),
                m = Number(arr[1].replace("月", ''));
            return [y, m];
        },
        ...
        event: function() {
            ...
            _this.$element
                ...
                .on("click", ARROW_MONTH, function() {
                    var y = Number(_this.$disMonth.text().replace("年", "")),
                        type = getClass(this);
                    y = type === "calendar-prev" ? y - 1 : y + 1;
        ...
        setData: function(data) {
            ...
            if (this.view === "date") {
                ...
                this.updateMonthView(this.$disMonth.text().replace("年", ""));
            }
        },
